...
```java
AdminAppsRequestsController adminAppsRequestsController = client.getAdminAppsRequestsController();
```
...
```java
CompletableFuture<ApiResponse<DefaultSuccessTemplate>> adminAppsRequestsListAsync(
    final String token,
    final Integer limit,
    final String cursor,
    final String teamId)
...
adminAppsRequestsController.adminAppsRequestsListAsync(token, null, null, null).thenAccept(result -> {
